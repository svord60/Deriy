...
# Карта для оплаты
CARD_NUMBER = os.environ.get("CARD_NUMBER", "2200700527205453")

# Курсы
STAR_RATE = float(os.environ.get("STAR_RATE", "1.5"))
USD_RATE = float(os.environ.get("USD_RATE", "84.0"))

# Цены на премиум
PREMIUM_PRICES = {
    "3months": {
        "rub": float(os.environ.get("PREMIUM_3M_RUB", "1124.11")),
        "usd": float(os.environ.get("PREMIUM_3M_USD", "14.12")),
        "name": "3 месяца"
    },
    "6months": {
        "rub": float(os.environ.get("PREMIUM_6M_RUB", "1498.81")),
        "usd": float(os.environ.get("PREMIUM_6M_USD", "14.12")),
        "name": "6 месяцев"
    },
    "1year": {
        "rub": float(os.environ.get("PREMIUM_1Y_RUB", "2716.59")),
        "usd": float(os.environ.get("PREMIUM_1Y_USD", "34.12")),
        "name": "1 год"
    }
}

# Ссылки
MAIN_PHOTO_ID = os.environ.get("MAIN_PHOTO_ID", "AgACAgIAAxkBAAFAAYFpVl91J1kMKJxRmeWE0cL1JL4bMwACTA1rG3xAsEokOAkz6UTdpAEAAwIAA3kAAzgE")
REPUTATION_CHANNEL = os.environ.get("REPUTATION_CHANNEL", "https://t.me/+3pbAABRgo1ljOTJi")
NEWS_CHANNEL = os.environ.get("NEWS_CHANNEL", "https://t.me/NewsDigistars")
SUPPORT_USER = os.environ.get("SUPPORT_USER", "@swordSar")
...
async def admin_settings_handler(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Для изменения используйте переменные окружения на хостинге.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")]
        ]),
        parse_mode="Markdown"
